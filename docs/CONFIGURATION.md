...
To use this preset, add `@exodus/linaria-babel` to your Babel configuration at the end of the presets list:
...
On top of the default template, you will need to install `@babel/preset-react`. This is because Linaria works with JSX syntax. Otherwise, preact will throw an error saying that `@exodus/linaria-webpack-loader` can't parse JSX. Don't forget to install all required `@linaria`-packages!.
...
+   "@babel/preset-react": "^7.8.3",
  ...
  "dependencies": {
+   "@exodus/linaria-babel": "^3.0.0",
+   "@exodus/linaria-core": "^3.0.0",
+   "@exodus/linaria-react": "^3.0.0",
+   "@exodus/linaria-webpack-loader": "^3.0.0",
    "preact": "^10.3.2",
...
export default config => {
  ...
  options.presets.push('@babel/preset-react', '@exodus/linaria-babel'); // Push the necessary presets
...
> You can also use the `styled` variant, importing from `@exodus/linaria-react`.
...
First, you will need to install `@exodus/linaria-babel` and `babel-preset-gatsby`. Then, create `babel.config.js` in the root of your project with the following contents:
